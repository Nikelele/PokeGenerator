    ...
    <>
      {Pokemon.error ? (
        ...
        <PokemonKachel
          ...
          stats={Pokemon.data && !Pokemon.loading && stats}
        />
      )}
    </>
  );
...
const PokemonKachel = (props) => {
  const { data, isLoading, moves, stats } = props;
  ...
  return (
    <div className={isShiny() ? "kachel shiny flipInY" : "kachel flipInY"}>
      {!isLoading && data ? (
        <>
          <div>
          ...
          <div>
            ...
            <ul key={getFront}>{getFront ? moves : stats}</ul>
          </div>
        </>
      ) : (
